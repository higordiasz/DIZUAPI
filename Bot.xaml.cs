...
using DIZUAPI.Controllers;
using DIZUAPI.Models;
using DIZUAPI.Models.Instagrams;
using DIZUAPI.Models.Blocks;
using DIZUAPI.Models.ActionModels;
using System.Windows.Media.Imaging;
using System.Windows.Documents;
using Block = DIZUAPI.Models.Blocks.Block;
using System.Windows.Media;
using DIZUAPI.WebInstagram;
        ...
        private async Task IniciarSistema()
        {
            ...
            if (File.Exists($@"{dir}\Config\dizutoken.txt"))
            {
                ...
                if (linhas.Length == 1)
                {
                    ...
                    {
                        await ConsoleMessage2("Erro ao conectar com a Dizu", Variaveis.Vermelho);
                        return;
                    }
                }
                else
            ...
            try
            {
                ...
                if (Directory.Exists($@"{dir}\logs"))
                {
                    ...
                    if (File.Exists($@"{dir}\logs\{data}.txt"))
                    {
                        ...
                        list.Add($"DIZUAPI {HorarioString()} {message}");
                        File.WriteAllLines($@"{dir}\logs\{data}.txt", list);
                        return;
                    }
                    else
                    {
                        string[] linhas = { $"DIZUAPI {HorarioString()} {message}" };
